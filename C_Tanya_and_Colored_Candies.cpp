...
vector<vector<int>> dp(100, vector<int>(2010));
string str;
int n;
int dpp(int i, int cnt)
{
    cnt = cnt - arr[i];
    if (cnt <= 0)
        return 0;
    ...
    for (int j = 1; j <= n; j++)
    {
        if (arr[i] < arr[j] and str[i - 1] != str[j - 1])
            ans = min(ans, dpp(j, cnt) + abs(j - i));
    }
    return dp[i][cnt] = ans;
}
int main()
