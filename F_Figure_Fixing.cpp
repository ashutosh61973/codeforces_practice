...
///////////////////////////////////////////////////////////////////////
bool is_bipartite(vector<vector<ll>> &g, ll n, vector<ll> &side)
{
    queue<ll> q;
    bool is_bi = true;
    for (int i = 1; i <= n; i++)
...
void solve()
{
    ...
    cin >> n >> m;
    // cout << n << " " << m << endl;
    vector<ll> ini(n + 1), tar(n + 1);
    ll ini_sum = 0, tar_sum = 0;
    for (int i = 1; i <= n; i++)
    {
        cin >> ini[i];
        ini_sum = (ini_sum + abs(ini[i])) % 2;
    }
    for (int i = 1; i <= n; i++)
    {
        cin >> tar[i];
        tar_sum = (tar_sum + abs(tar[i])) % 2;
    }
    // int parity = ((ini_sum % 2) != (tar_sum % 2));
    ...
    vector<ll> side(n + 10, -1);
    if (!(is_bipartite(g, n, side)))
    {
        cout << "YES" << endl;
        return;
    }
